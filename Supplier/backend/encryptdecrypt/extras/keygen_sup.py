...
def demonstrate_asymmetric_key_storage(password):
    """
    Create public and private asymmetric keys (2048 bits) for the manufacturer.
    You can Random password generation using strong secure random number generator
    ...
    try:
        ...
        # SERIALIZATION - converts private key which is in bytes in human readable form
        ...
        # Write public keys and private keys in the respective pem files
        with open("private_supplier.pem", 'wb') as key_file:
            key_file.write(pem_private)
        with open("public_supplier.pem", 'wb') as key_file:
