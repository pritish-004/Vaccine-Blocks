...
class Block:
    ...
    def is_valid_block(last_block, block):
        ...
        if block.last_hash != last_block.hash:
            raise Exception('The block last_hash if the received block is incorrect')

        if hex_to_binary(block.hash)[0:block.difficulty] != '0' * block.difficulty:
            raise Exception('The difficulty and the leading number of zeroes dont match')

        if abs(last_block.difficulty - block.difficulty) > 1:
            raise Exception('The difficulty value was not adjusted properly')
