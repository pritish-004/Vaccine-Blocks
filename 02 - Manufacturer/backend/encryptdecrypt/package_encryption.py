...
# set up logger - exception handling
...
def encrypt_package_data(sender_address,recipient,product_id1,product_id2,product_id3,product_id4,product_id5,package_id,password):
    ...
    - Generation of public and private RSA 2048
     bit keypair.(4096/8192 bits can also be used)
    - 
    """
    plain_text_list = []
    ...
    try:
        # LOAD KEYS
        with open("/home/pritish/Documents/python-blockchain/backend/encryptdecrypt/private_OEM.pem", "rb") as key_file:
        ...
        with open("/home/pritish/Documents/python-blockchain/backend/encryptdecrypt/public_Dist.pem", "rb") as key_file:
            public_key_Dist = serialization.load_pem_public_key(
                data=key_file.read(),
                backend=default_backend()
            )
        ...
        cipher_text_bytes = public_key_Dist.encrypt(
